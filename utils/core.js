...
module.exports = {
    ...
    /*读取某个文件夹下的所有文件夹
    @params folderDir 文件夹路径
    @params type 0为文件夹，1为文件
    最终结果为{path:[],size:}
    */
    readAllByFolder : function(folderDir,type){
        var folderNames = {path:[]};
        var fileNames = {path:[],sizeDic:{}};
       var pa =  fs.readdirSync(folderDir);
        pa.forEach(function(ele,index){  
        var info = fs.statSync(folderDir+"//"+ele);      
        if(info.isDirectory()){  
           // console.log("dir: "+ele); 
            folderNames.path.push(ele); 
            //readAllByFolder(folderDir+"//"+ele);  
        }else{  
           // console.log("file: "+ele);
            fileNames.path.push(ele);
            fileNames.sizeDic[ele] = info.size;
        }     
    });
    if (type==0){
        return folderNames;
